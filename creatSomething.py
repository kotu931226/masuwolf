...
class LoadLog:
    ...
    def find_something(self, word):
        ...
        if any(map(lambda x: x in word[5], ('COMINGOUT', 'VOTE', 'DIVINE', 'ESTIMATE'))):
            # Example of printing ['1', 'talk', '8', '1', '2', 'DIVINED Agent[01] HUMAN']
            # Example of printing [0:day, 1:type, 2:talk_id, 3:turn_num, 4:Agent_num, 5:talk_content]
...
def conv_onehot(label_matrix, max_num=92):
...
def main():
    load_log = LoadLog()
    onehot_t = np.array(load_log.emit_onehot_t())
    ...
    np.save('onehot_t.npy', onehot_t)
    np.save('onehot_y.npy', wolf_onehot)
